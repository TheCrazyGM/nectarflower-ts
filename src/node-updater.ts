...
function isValidNectarMetadata(obj: any): obj is NectarflowerJsonMetadata {
  return typeof obj === "object" && obj !== null && Array.isArray(obj.nodes);
}

const getNodesFromAccount = async (
  accountName: string,
  enableLogging: boolean
): Promise<NodeData> => {
  if (enableLogging) {
    console.log(`Workspaceing account metadata for ${accountName}...`);
  }

  const accounts: Account[] = await client.database.getAccounts([accountName]);
  ...
  try {
    ...
    if (parseError instanceof Error) {
      ...
      throw new Error("Failed to parse JSON metadata: Unknown error occurred");
    }
  }

  if (!isValidNectarMetadata(parsedMetadata)) {
    throw new Error(
...
const reinitializeDhive = (
  ...
  client = new Client(validNodes);
  return client;
};

const updateNodesFromAccount = async (
  accountName: string = "nectarflower",
  enableLogging: boolean = true
): Promise<Client> => {
  try {
    const nodeData = await getNodesFromAccount(accountName, enableLogging);
    const updatedClient = reinitializeDhive(nodeData, enableLogging);
    if (enableLogging) {
...
/**
 * Note: exporting client will only be useful if you decide to have live code samples, like the ones in the initial nectarflower-js
 */
